...
object HindiNumberConverter {
    ...
    /**
     * Converts a numeric currency amount into a long-form Hindi string.
     */
    fun convert(/** paise amount */ amount: Double): String {
    ...
    /**
     * Converts a numeric value into a concise Hindi text representation.
     */
    fun convertShort(/** raw value */ amount: Double): String {
        val whole = amount.toLong()
        ...
        val text = when {
            ...
            whole <= 199 -> "एक सौ " + (if ((whole % 100) > 0) {
                convertUnder100((whole % 100).toInt()) + " " 
            } else "")
            whole <= 999 -> {
            ...
            whole <= 9999999 -> {
                ...
                val r = whole % 100000
                convertUnder99(l) + " लाख " + (if (r > 0) {
                    convertUnder99999(r.toInt()) + " " 
                } else "")
            }
            else -> {
                val c = (whole / 10000000).toInt()
                val r = whole % 10000000
                convertUnder99(c) + " करोड़ " + (if (r > 0) {
                    convertUnder9999999(r.toInt()) + " " 
                } else "")
            }
        }
        return text.trim()
     ...
     */
    fun parseHindiNumbers(/** raw text */ text: String): Double {
        ...
        total += current + (modifier * 100)
        return total
    }

    private fun convertWhole(/** value */ n: Long): String = when {
        n == 0L -> "शून्य"
        n <= 19 -> ones[n.toInt()]
        n <= 99 -> convertUnder100(n.toInt())
        else -> convertUnder99(n.toInt())
    }

    private fun convertUnder100(/** value */ n: Int): String = when {
        n <= 19 -> ones[n]
        (n % 10) == 0 -> tens[n / 10]
        else -> tens[n / 10] + " " + ones[n % 10]
    }

    private fun convertUnder99(/** value */ n: Int): String = convertUnder100(n)

    private fun convertUnder999(/** value */ n: Int): String = when {
        n <= 99 -> convertUnder100(n)
        else -> ones[n / 100] + " सौ " + (if ((n % 100) > 0) convertUnder100(n % 100) else "")
    }

    private fun convertUnder99999(/** value */ n: Int): String = when {
        n <= 999 -> convertUnder999(n)
        else -> convertUnder99(n / 1000) + " हज़ार " + (if ((n % 1000) > 0) {
            convertUnder999(n % 1000)
        } else "")
    }

    private fun convertUnder9999999(/** value */ n: Int): String = when {
        n <= 99999 -> convertUnder99999(n)
        else -> convertUnder99(n / 100000) + " लाख " + (if ((n % 100000) > 0) {
            convertUnder99999(n % 100000)
        } else "")
    }
}
